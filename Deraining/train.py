...
for name, param in model_restoration.named_parameters():
    if ('sam'in name and'conv' in name) or 'concat' in name or 'tail' in name:
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        char = criterion_char(restored[0],target)
        edge = criterion_edge(restored[0],target)
        for j in range(len(restored)):
            if j > 0:
                char = char + criterion_char(restored[j],target)
                edge = edge + criterion_edge(restored[j],target)
        loss_char = char
        loss_edge = edge
